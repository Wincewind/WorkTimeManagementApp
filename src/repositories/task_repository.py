...
class TaskRepository:
    ...
    def get_weeks_tasks(self, user_id: int, week_num: int):
        sql = text("""SELECT tasks.id as task_id, task_date, duration_hours, duration_minutes, customers.name as customer_name,
    ...
    def get_task(self, task_id, user_id):
        sql = text("""SELECT id, task_date, duration_hours, duration_minutes, customer_id,
        project_id, task_type_id, invoiceable, note FROM tasks
        WHERE user_id =:user_id AND tasks.id=:task_id;""")
        return self._db.session.execute(sql, {'task_id':task_id,'user_id':user_id}).fetchone()
    
    def delete(self, task_id, user_id):
        sql_sel = text("SELECT FROM tasks WHERE user_id=:user_id AND id=:task_id;")
        if len(self._db.session.execute(sql_sel, {'task_id':task_id, 'user_id':user_id}).fetchall()) == 0:
            return False
    ...
    def edit(self, task_values: dict):
        sql = text("""UPDATE tasks SET duration_hours=:duration_hours, 
